    ...
    async with current_app.pool.acquire() as conn:
        ...
        if result:
            ...
            for u_id, info in multi_dict.items():
                info['permissions'] = [i['permission'] for i in multi_dict.getlist(u_id)]
                del info['permission']
...
async def manage_(file):
    ...
    async with current_app.pool.acquire() as conn:
        image_info = await conn.fetch(
            "SELECT Tags.id as tag_id,Images.source,Images.file,Images.extension,Images.under_review,Images.hidden,Images.is_nsfw, Images.uploader, Registered_user.name FROM Images LEFT JOIN Registered_user ON Registered_user.id=Images.uploader LEFT JOIN LinkedTags ON LinkedTags.image=Images.file LEFT JOIN Tags ON Tags.id=LinkedTags.tag_id WHERE Images.file=$1",
            file,
        )
        if not image_info:
    ...
    return await render_template(
        ...
        uploader_id=image_info[0]["uploader"],
